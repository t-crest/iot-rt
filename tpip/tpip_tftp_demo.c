...
void tftp_demo(){
    ...
    for(int i = 0; i < 20; i++){
        ...
        memset(bufout, 0, sizeof(bufout));
        memset(bufin, 0, sizeof(bufin));
        ans = tpip_tftp_send_rrq(1, filename, mode, bufout, tx_addr);
        if(!ans){
        ...
        if(packet_type == UNSUPPORTED){
            ...
            unpackip(&recv_ip_pck, bufin+14); //Extract data from buffer without Eth header               
            ...
            if ((recv_ip_pck.dstip & 0xFF) == 255){
                ...
                do{
                    ...
                    packet_type = mac_packet_type(rx_addr);
                    if(packet_type == UDP)
                    unpackip(&recv_ip_pck, bufin+14); 

                }while(recv_udp_packet.dstport == my_udp_port && packet_type == UDP);
